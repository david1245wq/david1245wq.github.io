(función(){
    ...
    var Ayudantes = {
      /**
       * Concatena propiedad y devuelve obj concatenado
       * @param o objeto a devolver con nuevos productos
       * @param O objeto a concatenar
       * @return devuelve el objeto con la propiedad de productos concatenados
       * @memberOf Ayudantes
       */
      fusionar: función (o, O) {
        para (propiedad var en O) {
          si (O.hasOwnProperty(propiedad)) {
            intentar {
              if (propiedad === 'productos') {
                o.productos = o.productos || [];
                o.productos = o.productos.concat(O.productos);
              } else if (tipo de o[propiedad] === 'objeto') {
                o[propiedad] = Helpers.merge(o[propiedad], O[propiedad]);
              } demás {
       ...
       * @memberOf Ayudantes
       */
      retornoSignificativo: función (P) {
        P.reverse();
    
        var x = {};
        var r = [];
    
        P.paraCada(función (p) {
          if (!x.hasOwnProperty(p.ruta)) {
            x[p.ruta] = verdadero;
            r.push(p);
          }
        });
      ...
      runCallback: función (devolución de llamada, argumentos) {
        //ejecuta la función de devolución de llamada si es una función o una cadena
        if (tipo de devolución de llamada === 'función') {
          callback.apply(nulo, argumentos);
        }
      }
    };
    
    la función está vacía (obj) {
      if (tipo de obj === 'indefinido' || obj === nulo) {
        devolver verdadero;
      }
    
      para (var prop en obj) {
        si (obj.hasOwnProperty(prop)) {
          falso retorno;
        }
      }
    
      devolver JSON.stringify(obj) === JSON.stringify({});
    }
    
    función depurar (cadena) {
      if (API.debug || Helpers.storage.getItem('debug')) {
        var args = Array.prototype.slice.call(argumentos);
        if (tipo de cadena === 'cadena') {
          args.unshift('[API FastSpring] ' + args.shift());
        }
    ...
    var almacenamientoAccesible = verdadero;
    intentar {
      ventana.localStorage.setItem('fs','fs');
      ventana.localStorage.removeItem('fs');
    } captura(e) {
      almacenamientoAccesible = falso;
    }
    
    Object.defineProperty(Ayudantes, 'almacenamiento', {
      obtener: función _getStorage() {
        if (almacenamientoAccesible && ('almacenamiento local' en la ventana)) {
          devolver ventana.localStorage;
        } demás {
          if (tipo de _getStorage.storage === 'indefinido') {
            _getStorage.almacenamiento = {
              _datos : {},
              setItem: función(id, val) { return this._data[id] = String(val); },
              getItem: función (id) {devolver this._data.hasOwnProperty (id)? this._data[id] : indefinido; },
              removeItem: function(id) { return eliminar this._data[id]; },
              claro: función() {devuelve this._data = {}; }
            };
          }
          devolver _getStorage.almacenamiento;
        }
      }
    });
    ;
    ...
    var Girador = {
      adjuntar: función () {
        si (API.isInlineCheckout) {
          devolver;
        }
        this.spinSVG.style.position = 'absoluto';
        ...
        this.spinSVG.style.display = 'bloquear';
      ...
      eliminar: función () {
        ...
        if (tipo de esto.spinSVG! == 'indefinido') {
          this.spinSVG.style.display = 'ninguno';
        }
      }
    };
    ;
    ...
    var API = {
      ...
      asignarAttributeValues: función ($fscContainer) {
        /** @property {booleano} this.animateFader - predeterminado: verdadero */
        this.animateFader = ($fscContainer.getAttribute('data-animate-fader') || 'true');
    
        this.beforeRequests = $fscContainer.getAttribute('datos-antes-de-solicitudes-devolución de llamada');
        this.afterRequests = $fscContainer.getAttribute('datos-después-de-solicitudes-devolución de llamada');
    
        this.beforeMarkup = $fscContainer.getAttribute('datos-antes-de-marcación-devolución de llamada');
        ...
        this.dataCallback = $fscContainer.getAttribute('datos-datos-callback');
        this.errorCallback = $fscContainer.getAttribute('datos-error-callback');
        this.validationCallback = $fscContainer.getAttribute('validación-de-datos-callback');
        this.markupHelpers = $fscContainer.getAttribute('data-markup-helpers-callback');
    
        this.popupClosed = $fscContainer.getAttribute('datos-emergente-cerrado');
        this.popupWebhookReceived = $fscContainer.getAttribute('datos-popup-webhook-recibido');
        this.popupEventReceived = $fscContainer.getAttribute('datos-evento-emergente-recibido');
    
        this.storefront = $fscContainer.getAttribute('data-storefront');
        this.debug = ($fscContainer.getAttribute('data-debug') === 'true');
        this.aKey = $fscContainer.getAttribute('clave-de-acceso-a-datos');
        this.continuous = ($fscContainer.getAttribute('datos-continuos') === 'true');
      ...
      inicio: función () {
        ...
        this.isInlineCheckout = falso;
        this.checkoutMode = 'redireccionar';
        if (this.storefront.match(/pinhole/) !== nulo || this.storefront.match(/popup/) !== nulo) {
          this.checkoutMode = 'emergente';
        }
        if (this.storefront.match(/inapp/)!== nulo) {
          this.checkoutMode = 'inapp';
        }
        si (this.storefront.match(/embedded/) !== nulo) {
          this.checkoutMode = 'emergente'; // La variable checkoutMode es utilizada por la API del constructor. Estamos usando el flujo emergente detrás de escena para en línea.
          this.isInlineCheckout = verdadero;
        }
    
        debug('Modo de pago:', this.checkoutMode);
    
        this.testMode = falso;
    
        if ((/\.test\./).test(este.escaparate)) {
          this.testMode = verdadero;
        }
        else if ((/(localhost:[^?]*\*)/).test(this.storefront)) {
          this.testMode = verdadero;
        }
    
        debug('Modo de prueba:', this.testMode);
    
        if (this.aKey! == nulo) {
          debug('Clave de acceso:', this.aKey);
        }
        ...
        APIInitialized = verdadero;
      },
    
      /**
       * Obtiene la sesión de LocalStorage si el modo continuo está habilitado
       * @memberOf API
       */
      obtención continua: función () {
          debug(' -> Comprobando la sesión existente');
    
          var serie = nulo;
          var maySerial = Helpers.storage.getItem('fscSerial-' + this.storefront);
    
          si (tal vez en serie) {
            tal vezSerial = JSON.parse(tal vezSerial);
            if (maybeSerial.hasOwnProperty('sesión') && maybeSerial.hasOwnProperty('expires')) {
                serial = tal vezSerial.session;
              }
            }
            devolver serie;
      },
      ...
      /**
       * Almacena la sesión en almacenamiento local para modo continuo
       * @param serie
       * @memberOf API
       */
      Tienda continua: función (serie) {
        debug(' <- Escribir en el almacenamiento:', serial);
        Helpers.storage.setItem('fscSerial-' + this.storefront, JSON.stringify(serial));
      },
    
      /**
       * Elimina la sesión en el almacenamiento local para el modo continuo
       * @memberOf API
       */
      reinicio continuo: función () {
        debug(' <- Restableciendo sesión');
        Helpers.storage.removeItem('fscSerial-' + this.storefront);
        this.sessionID = nulo;
      },
    
      /**
       * Sesión de actualizaciones en almacenamiento local para modo continuo
       * @memberOf API
       */
      actualizarSesión: función () {
        debug(' <- Sesión de actualización');
        carga útil var = {};
